...
/**
  ...
  *
  *
  * This user manual describes the usage of NUC230_240 Series MCU device driver
  *
  * <b>Disclaimer</b>
  *
...
#include "system_NUC230_240.h"          /* System                                                 */
 ...
 * |        |          |1 = Set TxRqst bit.
 * |        |          |Note: If a transmission is requested by programming bit TxRqst/NewDat in the IFn Command Mask
 * |        |          |Register, bit TxRqst in the IFn Message Control Register will be ignored.
 ...
 * |        |          |IntPnd and NewDat.
 * |        |          |The values of these bits transferred to the IFn Message Control Register always reflect the
 ...
 * |        |          |1 = Transfer Identifier + Dir (CAN_IFn_ARB2[13]) + Xtd (CAN_IFn_ARB2[14]) + MsgVal
 * |        |          |(CAN_IFn_APB2[15]) to Message Object.
 * |        |          |Read Operation:
 * |        |          |0 = Arbitration bits unchanged.
 ...
 * |        |          |0 = Read: Transfer data from the Message Object addressed by the Command Request Register into
 * |        |          |the selected Message Buffer Registers.
 * |        |          |1 = Write: Transfer data from the selected Message Buffer Registers to the Message Object
 ...
 * |        |          |0 = The corresponding bit in the identifier of the message object cannot inhibit the match in
 ...
 * |        |          |Note: When 11-bit ("standard") Identifiers are used for a Message Object, the identifiers of
 * |        |          |received Data Frames are written into bits ID28 to ID18 (CAN_IFn_ARB2[12:2]).
 * |        |          |For acceptance filtering, only these bits together with mask bits Msk28 to Msk18
 * |        |          |(CAN_IFn_MASK2[12:2]) are considered.
 * @var CAN_IF_T::ARB1
 ...
 * |        |          |On reception of a Data Frame with matching identifier, that message is stored in this Message
 * |        |          |Object.
 * |        |          |1 = Direction is transmit.
 * |        |          |On TxRqst, the respective Message Object is transmitted as a Data Frame.
 * |        |          |On reception of a Remote Frame with matching identifier, the TxRqst bit (CAN_IFn_CMASK[2]) of
 * |        |          |this Message Object is set (if RmtEn (CAN_IFn_MCON[9]) = one).
 ...
 * |        |          |Note: The application software must reset the MsgVal bit of all unused Messages Objects during
 * |        |          |the initialization before it resets bit Init (CAN_CON[0]).
 * |        |          |This bit must also be reset before the identifier Id28-0 (CAN_IFn_ARB1/2), the control bits Xtd
 * |        |          |(CAN_IFn_ARB2[14]), Dir (CAN_IFn_APB2[13]), or the Data Length Code DLC3-0 (CAN_IFn_MCON[3:0])
 * |        |          |are modified, or if the Messages Object is no longer required.
 * @var CAN_IF_T::MCON
 ...
 * |        |          |corresponding objects with the same identifier at other nodes.
 * |        |          |When the Message Handler stores a data frame, it will write the DLC to the value given by the
 * |        |          |received message.
 ...
 * |        |          |during a reception while the Data 7 byte is the last.
 * |        |          |When the Message Handler stores a Data Frame, it will write all the eight data bytes into a
 * |        |          |Message Object.
 * |        |          |If the Data Length Code is less than 8, the remaining bytes of the Message Object will be
 * |        |          |overwritten by unspecified values.
 * |[7]     |EoB       |End Of Buffer
 * |        |          |0 = Message Object belongs to a FIFO Buffer and is not the last Message Object of that FIFO
 * |        |          |Buffer.
 * |        |          |1 = Single Message Object or last Message Object of a FIFO Buffer.
 * |        |          |Note: This bit is used to concatenate two or more Message Objects (up to 32) to build a FIFO
 * |        |          |Buffer.
 ...
 * |        |          |Note: If the UMask bit is set to one, the Message Object's mask bits have to be programmed
 * |        |          |during initialization of the Message Object before MsgVal bit (CAN_IFn_APB2[15]) is set to one.
 * |[13]    |IntPnd    |Interrupt Pending
 * |        |          |0 = This message object is not the source of an interrupt.
 * |        |          |1 = This message object is the source of an interrupt.
 * |        |          |The Interrupt Identifier in the Interrupt Register will point to this message object if there is
 ...
 * |        |          |1 = The Message Handler stored a new message into this object when NewDat was still set, the CPU
 * |        |          |has lost a message.
 * |[15]    |NewDat    |New Data
 ...
 * |        |          |1 = Enabled - An interrupt will be generated when a message transfer is successfully completed
 * |        |          |or a CAN bus error is detected.
 * |[3]     |EIE       |Error Interrupt Enable Control
 * |        |          |0 = Disabled - No Error Status Interrupt will be generated.
 * |        |          |1 = Enabled - A change in the bits BOff (CAN_STATUS[7]) or EWarn (CAN_STATUS[6]) in the Status
 ...
 * |        |          |This field will be cleared to '0' when a message has been transferred (reception or
 ...
 * |        |          |1 = Since this bit was last reset by the CPU, a message has been successfully (error free and
 * |        |          |acknowledged by at least one other node) transmitted.
 ...
 * |        |          |0x01-0x3F: The value by which the oscillator frequency is divided for generating the bit time
 * |        |          |quanta.
 ...
 * |        |          |interrupt with the highest priority, disregarding their chronological order.
 * |        |          |An interrupt remains pending until the application software has cleared it.
 * |        |          |If IntId is different from 0x0000 and IE (CAN_IFn_MCON[1]) is set, the IRQ interrupt signal to
 * |        |          |the EIC is active.
 * |        |          |The interrupt remains active until IntId is back to value 0x0000 (the cause of the interrupt is
 * |        |          |reset) or until IE is reset.
 * |        |          |The Status Interrupt has the highest priority.
 * |        |          |Among the message interrupts, the Message Object' s interrupt priority decreases with increasing
 * |        |          |message number.
 ...
 * |        |          |The actual interpretation by the hardware is that one more than the value programmed by BRPE
 * |        |          |(MSBs) and BTIME (LSBs) is used.
 * @var CAN_T::TXREQ1
 ...
 * |        |          |0 = No new data has been written into the data portion of this Message Object by the Message
 * |        |          |Handler since the last time this flag was cleared by the application software.
 * |        |          |1 = The Message Handler or the application software has written new data into the data portion
 * |        |          |of this Message Object.
 * @var CAN_T::NDAT2
 ...
 */
    ...
    __IO uint32_t ERR;           /* Offset: 0x08  Error Counter Register                                             */
    __IO uint32_t BTIME;         /* Offset: 0x0C  Bit Timing Register                                                */
    __IO uint32_t IIDR;          /* Offset: 0x10  Interrupt Identifier Register                                      */
    ...
    __IO CAN_IF_T IF[2];         /* Offset: 0x20  CAN Interface Control Registers                                    */
    __I  uint32_t RESERVE1[8];  
    __IO uint32_t TXREQ1;        /* Offset: 0x100  Transmission Request Register 1                                   */
    __IO uint32_t TXREQ2;        /* Offset: 0x104  Transmission Request Register 2                                   */
    __I  uint32_t RESERVE2[6];  
    __IO uint32_t NDAT1;         /* Offset: 0x120  New Data Register 1                                               */
    __IO uint32_t NDAT2;         /* Offset: 0x124  New Data Register 2                                               */
    __I  uint32_t RESERVE3[6];  
    __IO uint32_t IPND1;         /* Offset: 0x140  Interrupt Pending Register 1                                      */
    __IO uint32_t IPND2;         /* Offset: 0x144  Interrupt Pending Register 2                                      */
    __I  uint32_t RESERVE4[6];  
    __IO uint32_t MVLD1;         /* Offset: 0x160  Message Valid Register 1                                          */
    __IO uint32_t MVLD2;         /* Offset: 0x164  Message Valid Register 2                                          */
...
#define CAN_IF_DAT_A2_DATA3_Pos    8                                    /*!< CAN_IF_T::DAT_A1: DATA3 Position */
#define CAN_IF_DAT_A2_DATA3_Msk    (0xFFul << CAN_IF_DAT_A2_DATA3_Pos)  /*!< CAN_IF_T::DAT_A1: DATA3 Mask     */

#define CAN_IF_DAT_A2_DATA2_Pos    0                                    /*!< CAN_IF_T::DAT_A1: DATA2 Position */
#define CAN_IF_DAT_A2_DATA2_Msk    (0xFFul << CAN_IF_DAT_A2_DATA2_Pos)  /*!< CAN_IF_T::DAT_A1: DATA2 Mask     */
 ...
 * |        |          |The bit default value is set by flash controller user configuration register CFOSC
 * |        |          |(Config0[26:24]).
 * |        |          |When the default clock source is from external 4~24 MHz high speed crystal, this bit is set to 1
 * |        |          |automatically.
 ...
 * |        |          |When the chip wakes up from Power-down mode, the clock control will delay certain clock cycles
 * |        |          |to wait system clock stable.
 ...
 * |        |          |When this bit is set to 1, Power-down mode is enabled and chip Power-down behavior willdepends
 * |        |          |on the PD_WAIT_CPU bit.
 * |        |          |(a) If the PD_WAIT_CPU is 0, then the chip enters Power-down mode immediately after the
 * |        |          |PWR_DOWN_EN bit set.
 * |        |          |(b) if the PD_WAIT_CPU is 1, then the chip keeps active till the CPU sleep mode is also active
 * |        |          |and then the chip enters Power-down mode. (recommend)
 ...
 * |        |          |In Power-down mode, external 4~24 MHz high speed crystal oscillator and the internal 22.1184 MHz
 * |        |          |high speed oscillator will be disabled in this mode, 
 * |        |          |but the external 32.768 kHz low speed crystal and internal 10 kHz low speed oscillator are not
 * |        |          |controlled by Power-down mode.
 * |        |          |In Power- down mode, the PLL and system clock are disabled, and ignored the clock source
 * |        |          |selection.
 * |        |          |The clocks of peripheral are not controlled by Power-down mode, if the peripheral clock source
 * |        |          |is from external 32.768 kHz low speed crystal oscillator or the internal 10 kHz low speed
 * |        |          |oscillator.
 ...
 * |        |          |1 = Chip enters Power- down mode when the both PD_WAIT_CPU and PWR_DOWN_EN bits are set to 1 and
 ...
 * |        |          |This bit is used to control the RTC APB clock only, The RTC peripheral clock source is selected
 * |        |          |from RTC_SEL_10K(CLKSEL2[18]).
 ...
 * |        |          |(CLKSEL[2:0]).
 * |        |          |When user switch system clock, the system clock source will keep old clock until the new clock
 * |        |          |is stable.
 * |        |          |During the period that waiting new clock stable, this bit will be an index shows system clock
 * |        |          |source is not match as user wanted.
 * @var CLK_T::CLKSEL0
 ...
 * |        |          |1. Before clock switching, the related clock sources (both pre-select and new-select) must be
 * |        |          |turn on.
 * |        |          |2. The 3-bit default value is reloaded from the value of CFOSC (Config0[26:24]) in user
 ...
 * |        |          |Note2: if SysTick clock source is not from HCLK (i.e. SYST_CSR[2] = 0), SysTick clock source
 * |        |          |must less than or equal to HCLK/2.
 * @var CLK_T::CLKSEL1
 ...
 * |        |          |The clock source of PWM0 and PWM1 is defined by PWM01_S (CLKSEL1[29:28]) and PWM01_S_E
 * |        |          |(CLKSEL2[8]).
 ...
 * |        |          |The clock source of PWM2 and PWM3 is defined by PWM23_S (CLKSEL1[31:30]) and PWM23_S_E
 * |        |          |(CLKSEL2[9]).
 ...
 * |        |          |The clock source of PWM4 and PWM5 is defined by PWM45_S (CLKSEL2[5:4]) and PWM45_S_E
 * |        |          |(CLKSEL2[10]).
 ...
 * |        |          |The clock source of PWM6 and PWM7 is defined by PWM67_S (CLKSEL2[7:6]) and PWM67_S_E
 * |        |          |(CLKSEL2[11]).
 ...
 * |        |          |For Master mode, I2S_BCLK and I2S_LRCLK pins are output mode and send bit clock from NuMicro
 * |        |          |NUC230_240 series to Audio CODEC chip.
 * |        |          |In Slave mode, I2S_BCLK and I2S_LRCLK pins are input mode and I2S_BCLK and I2S_LRCLK signals are
 * |        |          |received from outer Audio CODEC chip.
 * |        |          |0 = Master mode.
 * |        |          |1 = Slave mode.
 ...
 * |        |          |When the count of received data word(s) in buffer is equal to or higher than threshold level,
 * |        |          |RXTHF (I2SSTATUS[10]) will be set.
 ...
 * |        |          |If MCLKEN is set to 1, I2S controller will generate master clock on I2S_MCLK pin for external
 * |        |          |audio devices.
 * |        |          |0 = Master clock Disabled.
 * |        |          |1 = Master clock Enabled.
 * |[16]    |RCHZCEN   |Right Channel Zero Cross Detection Enable
 * |        |          |If this bit is set to 1, when right channel data sign bit change or next shift data bits are all
 * |        |          |0 then RZCF flag in I2SSTATUS register is set to 1.
 ...
 * |        |          |If this bit is set to 1, when left channel data sign bit changes or next shift data bits are all
 * |        |          |0 then LZCF flag in I2SSTATUS register is set to 1.
 * |        |          |This function is only available in transmit operation.
 * |        |          |0 = Left channel zero cross detection Disabled.
 * |        |          |1 = Left channel zero cross detection Enabled.
 * |[18]    |CLR_TXFIFO|Clear Transmit FIFO
 * |        |          |Write 1 to clear transmit FIFO, internal pointer is reset to FIFO start point, and TX_LEVEL[3:0]
 * |        |          |returns to 0 and
 ...
 * |        |          |Write 1 to clear receive FIFO, internal pointer is reset to FIFO start point, and RX_LEVEL[3:0]
 * |        |          |returns 0 and receive FIFO becomes empty.
 * |        |          |This bit is cleared by hardware automatically. Returns 0 on read.
 * |[20]    |TXDMA     |Enable Transmit DMA
 * |        |          |When TX DMA is enabled, I2S request DMA to transfer data from SRAM to transmit FIFO if FIFO is
 * |        |          |not full.
 * |        |          |0 = TX DMA Disabled.
 * |        |          |1 = TX DMA Enabled.
 * |[21]    |RXDMA     |Enable Receive DMA
 * |        |          |When RX DMA is enabled, I2S requests DMA to transfer data from receive FIFO to SRAM if FIFO is
 * |        |          |not empty.
 * |        |          |0 = RX DMA Disabled.
 * |        |          |1 = RX DMA Enabled.
 * |[23]    |RXLCH     |Receive Left Channel Enable
 * |        |          |When monaural format is selected (MONO = 1), I2S controller will receive right channel data if
 * |        |          |RXLCH is set to 0,
 ...
 * |        |          |Interrupt occurs if this bit is set to 1 and the count of data words in transmit FIFO is less
 * |        |          |than TXTH (I2SCON[11:9]).
 * |        |          |0 = Interrupt Disabled.
 * |        |          |1 = Interrupt Enabled.
 ...
 * |        |          |When receive FIFO is full and hardware attempt to write data to receive FIFO, this bit will be
 * |        |          |set to 1, data in 1st buffer will be overwrote.
 * |        |          |0 = No overflow.
 * |        |          |1 = Overflow.
 * |        |          |Note: Write 1 to clear this bit to 0.
 ...
 * |        |          |When the count of data stored in transmit-FIFO is equal to or less than threshold value set in
 * |        |          |TXTH (I2SCON[11:9]).
 ...
 * |        |          |CON_SEL(SC_CTL[5:4]) will be set to 00 automatically, otherwise if the TS is inverse convention,
 * |        |          |and CON_SEL (SC_CTL[5:4]) will be set to 11.
 ...
 * |        |          |After hardware received first data and stored it at buffer, hardware will decided the convention
 * |        |          |and change the CON_SEL (SC_CTL[5:4]) bits automatically.
 ...
 * |        |          |When the number of bytes in the receiving buffer equals the RX_FTRI_LEV, the RDA_IF will be set
 * |        |          |(if IER [RDA_IEN] is enabled, an interrupt will be generated).
 ...
 * |        |          |According to ISO7816-3, in T = 0 mode, software must fill 15 (real block guard time = 16.5) to
 * |        |          |this field; in T = 1 mode, software must fill 21 (real block guard time = 22.5) to it.
 * |        |          |Note:
 ...
 * |        |          |Software can configure the 24 bit timer by setting SC_TMR0 [23:0] and configure the 8 bit timer
 * |        |          |by setting SC_TMR1[7:0].
 ...
 * |        |          |This field indicates the maximum number of receiver retries that are allowed when parity error
 * |        |          |has occurred
 ...
 * |        |          |00 = De-bounce sample card insert once per 384 (128 * 3) engine clocks and de-bounce sample card
 * |        |          |removal once per 128 engine clocks.
 * |        |          |01 = De-bounce sample card insert once per 192 (64 * 3) engine clocks and de-bounce sample card
 * |        |          |removal once per 64 engine clocks.
 * |        |          |10 = De-bounce sample card insert once per 96 (32 * 3) engine clocks and de-bounce sample card
 * |        |          |removal once per 32 engine clocks.
 * |        |          |11 = De-bounce sample card insert once per 48 (16 * 3) engine clocks and de-bounce sample card
 * |        |          |removal once per 16 engine clocks.
 * |[30]    |SYNC      |SYNC Flag Indicator
 * |        |          |Due to synchronization, software should check this bit before writing a new value to RX_ERETRY
 * |        |          |and TX_ERETRY.
 ...
 * |        |          |When TX_RST is set, all the bytes in the transmit buffer and TX internal state machine will be
 * |        |          |cleared.
 * |        |          |0 = No effect.
 ...
 * |        |          |When RX_RST is set, all the bytes in the receiver buffer and Rx internal state machine will be
 * |        |          |cleared.
 * |        |          |0 = No effect.
 ...
 * |        |          |INIT_IS(SC_ISR[8]) will be set to 1.
 * |        |          |Note2:
 * |        |          |This field will be cleared by TX_RST(SC_ALTCTL[0]) and RX_RST(SC_ALTCTL[1]), so don't fill this
 * |        |          |bit, TX_RST(SC_ALTCTL[0]), and RX_RST(SC_ALTCTL[1]) at the same time.
 * |        |          |Note3:
 * |        |          |If SC_CEN(SC_CTL[0]) is not enabled, this filed cannot be programmed.
 ...
 * |        |          |Note2:
 * |        |          |This field will be cleared by TX_RST(SC_ALTCTL[0]) and RX_RST(SC_ALTCTL[1]), so don't fill this
 * |        |          |bit, TX_RST, and RX_RST at the same time.
 * |        |          |Note3:
 * |        |          |If SC_CEN(SC_CTL[0]) is not enabled, this filed cannot be programmed.
 ...
 * |        |          |Once the counter decrease to 1 and no new data is received or CPU does not read data by reading
 * |        |          |SC_RBR buffer, a receiver time-out interrupt INT_RTMR will be generated(if RTMR_IE(SC_IER[9]) = 1).
 ...
 * |        |          |This field is used for received data reaching trigger level RX_FTRI_LEV (SC_CTL[7:6]) interrupt
 * |        |          |enable.
 ...
 * |        |          |RX_EBR_F(SC_SR[6]), frame error RX_EFR_F(SC_SR[5]), parity error RX_EPA_F(SC_SR[4]), receiver
 * |        |          |buffer overflow error RX_OVER_F(SC_SR[0]), transmit buffer overflow error TX_OVER_F(SC_SR[8]),
 * |        |          |receiver retry over limit error RX_OVER_REERR(SC_SR[22]) and transmitter retry over limit error
 * |        |          |TX_OVER_REERR(SC_SR[30]).
 ...
 * |        |          |If software reads data from SC_RBR and receiver buffer data byte number is less than RX_FTRI_LEV
 * |        |          |(SC_CTL[7:6]), this bit will be cleared automatically.
 ...
 * |        |          |The transfer error states is at SC_SR register which includes receiver break error
 * |        |          |RX_EBR_F(SC_SR[6]), 
 * |        |          |frame error RX_EFR_F(SC_TRSR[5]), parity error RX_EPA_F(SC_TRSR[4]) and receiver buffer overflow
 * |        |          |error RX_OVER_F(SC_TRSR[0]), 
 * |        |          |transmit buffer overflow error TX_OVER_F(SC_TRSR[8]), receiver retry over limit error
 * |        |          |RX_OVER_REERR(SC_TRSR[22]) and transmitter retry over limit error TX_OVER_REERR(SC_TRSR[30]).
 ...
 * |        |          |This field is used for activation (ACT_EN(SC_ALTCTL[3])), deactivation (DACT_EN (SC_ALTCTL[2]))
 * |        |          |and warm reset (WARST_EN (SC_ALTCTL[4])) sequence interrupt status flag.
 * |        |          |Note: This bit is read only, but it can be cleared by writing 1 to it.
 ...
 * |        |          |If software wants to clear this bit, software must read all receiver buffer remaining data by
 * |        |          |reading SC_RBR buffer,.
 ...
 * |        |          |Note2:
 * |        |          |If CPU sets receiver retries function by setting RX_ERETRY_EN(SC_CTL[19]) , hardware will not
 * |        |          |set this flag.
 ...
 * |        |          |If TX buffer is full, an additional write to THR(SC_THR[7:0]) will cause this bit be set to "1"
 * |        |          |by hardware.
 * |        |          |Note: This bit is read only, but it can be cleared by writing 1 to it.
 ...
 * |        |          |When one byte of TX Buffer is transferred to transmitter shift register, TX_POINT_F decreases
 * |        |          |one.
 ...
 * |        |          |has completed.
 * |        |          |1 = This bit is set by hardware when TX transfer is in active and the STOP bit of the last byte
 ...
 * |        |          |Note: When operating at activation, warm reset or deactivation mode, this bit will be changed
 * |        |          |automatically.
 ...
 * |        |          |Note: When operating in activation, warm reset or deactivation mode, this bit will be changed
 * |        |          |automatically.
 * |        |          |So don't fill this field when operating in these modes.
 ...
 * |        |          |POW_EN(SC_PINCSR[0]).
 * |        |          |POW_INV (SC_PINCSR[11]) is bit 1 and POW_EN(SC_PINCSR[0]) is bit 0 for SC_POW_Pin as high or low
 * |        |          |voltage selection.
 * |        |          |00 = SC_POW_ Pin is 0.
 * |        |          |01 = SC_POW _Pin is 1.
 * |        |          |10 = SC_POW _Pin is 1.
 ...
 * |        |          |0 = Parity bit is generated or checked between the "last data word bit" and "stop bit" of the
 * |        |          |serial data.
...
#define SC_TRSR_TX_POINT_F_Msk              (3ul << SC_TRSR_TX_POINT_F_Pos)  /*!< SC_T::TRSR: TX_POINT_F Mask */
...
#define SC_TRSR_RX_POINT_F_Msk              (3ul << SC_TRSR_RX_POINT_F_Pos)     /*!< SC_T::TRSR: RX_POINT_F Mask */
...
#define SC_UACTL_UA_MODE_EN_Msk            (1ul << SC_UACTL_UA_MODE_EN_Pos) /*!< SC_T::UACTL: UA_MODE_EN Mask */
 ...
 * |        |          |The RSTS_POR flag is set by the "reset signal" from the Power-On Reset (POR) controller or bit
 * |        |          |CHIP_RST (IPRSTC1[0]) to indicate the previous reset source.
 ...
 * |        |          |The RSTS_RESET flag is set by the "Reset Signal" from the /RESET pin to indicate the previous
 * |        |          |reset source.
 * |        |          |0 = No reset from /RESET pin.
 * |        |          |1 = The Pin /RESET had issued the reset signal to reset the system.
 * |        |          |Note: Write 1 to clear this bit to 0.
 * |[2]     |RSTS_WDT  |Watchdog Reset Flag
 * |        |          |The RSTS_WDT flag is set by The "Reset Signal" from the Watchdog Timer to indicate the previous
 * |        |          |reset source
 ...
 * |        |          |The RSTS_LVR flag Is Set By The "Reset Signal" From The Low-Voltage-Reset Controller To Indicate
 ...
 * |        |          |SYSRESETREQ (AIRCR[2], Application Interrupt and Reset Control Register, address = 0xE000ED0C)
 * |        |          |in system control registers of Cortex-M0 kernel.
 * |        |          |Note: Write 1 to clear this bit to 0.
 ...
 * |        |          |Setting this bit will only reset the CPU kernel and Flash Memory Controller(FMC), and this bit
 * |        |          |will automatically return 0 after the two clock cycles.
 ...
 * |        |          |The default value is set by flash memory controller user configuration register CBODEN
 * |        |          |(Config0[23]) bit.
 ...
 * |        |          |BOD interrupt will keep till to the BOD_EN set to 0. BOD interrupt can be blocked by disabling
 * |        |          |the NVIC BOD interrupt or disabling BOD function (set BOD_EN low).
 * |        |          |1 = Brown-out "RESET" function Enabled.
 * |        |          |Note1: While the Brown-out Detector function is enabled (BOD_EN high) and BOD reset function is
 * |        |          |enabled (BOD_RSTEN high), BOD will assert a signal to reset chip when the detected voltage is
 * |        |          |lower than the threshold (BOD_OUT high).
 * |        |          |Note2: The default value is set by flash controller user configuration register CBORST
 * |        |          |(Config0[20]) bit.
 ...
 * |        |          |setting or the VDD is raised up through the voltage of BOD_VL setting, this bit is set to 1 and
 * |        |          |the Brown-out interrupt is requested if Brown-out interrupt is enabled.
 * |        |          |Note: Write 1 to clear this bit to 0.
 ...
 * |        |          |0 = Brown-out Detector output status is 0. It means the detected voltage is higher than BOD_VL
 * |        |          |setting or BOD_EN is 0.
 * |        |          |1 = Brown-out Detector output status is 1. It means the detected voltage is lower than BOD_VL
 * |        |          |setting. If the BOD_EN is 0, BOD function disabled, this bit always responds to 0.
 ...
 * |        |          |1 = Low Voltage Reset function Enabled - After enabling the bit, the LVR function will be active
 * |        |          |with 100us delay for LVR output stable (default).
 * |        |          |Note: This bit is write protected bit. Refer to the REGWRPROT register.
 * @var GCR_T::TEMPCR
 ...
 * |        |          |Note: After this bit is set to 1, the value of temperature can be obtained from ADC conversion
 * |        |          |result by ADC channel selecting channel 7 and alternative multiplexer channel selecting
 * |        |          |temperature sensor.
 ...
 * |        |          |When powered on, the POR circuit generates a reset signal to reset the whole chip function, but
 * |        |          |noise on the power may cause the POR active again.
 ...
 * |        |          |Bit EBI_HB_EN[4] (ALT_MFP[20]), EBI_EN (ALT_MFP[11]), PA1_SC0RST (ALT_MFP1[3]) and GPA_MFP[1]
 * |        |          |determine the PA.1 function.
 ...
 * |        |          |Bits EBI_HB_EN[3] (ALT_MFP[19]), EBI_EN (ALT_MFP[11]), PA2_SC0CLK (ALT_MFP1[0]) and GPA_MFP[2]
 * |        |          |determine the PA.2 function.
 * |        |          |(EBI_HB_EN[3], EBI_EN, PA2_SC0CLK, GPA_MFP2) value and function mapping is as following list.
 * |        |          |(0, 0, 0, 0) = GPIO function is selected.
 * |        |          |(0, 0, 0, 1) = ADC2 function is selected.
 * |        |          |(0, 0, 1, 1) = SC0_CLK/UART3_TXD function is selected.
 * |        |          |(1, 1, 0, 1) = AD11 function is selected.         
 * |[3]     |GPA_MFP3  |PA.3 Pin Function Selection   
 * |        |          |Bits EBI_HB_EN[2] (ALT_MFP[18]), EBI_EN (ALT_MFP[11]), PA3_SC0DAT (ALT_MFP1[1]) and GPA_MFP[3]
 * |        |          |determine the PA.3 function.
 * |        |          |(EBI_HB_EN[2], EBI_EN, PA3_SC0DAT, GPA_MFP3) value and function mapping is as following list.
 * |        |          |(0, 0, 0, 0) = GPIO function is selected.
 * |        |          |(0, 0, 0, 1) = ADC3 function is selected.
 * |        |          |(0, 0, 1, 1) = SC0_DAT/UART3_RXD function is selected.
 * |        |          |(1, 1, 0, 1) = AD10 function is selected.
 * |[4]     |GPA_MFP4  |PA.4 Pin Function Selection     
 * |        |          |Bits EBI_HB_EN[1] (ALT_MFP[17]), EBI_EN (ALT_MFP[11]), PA4_SC1PWR (ALT_MFP1[7]) and GPA_MFP[4]
 * |        |          |determine the PA.4 function.
 ...
 * |        |          |Bits EBI_HB_EN[0] (ALT_MFP[16]), EBI_EN (ALT_MFP[11]),PA5_SC1RST (ALT_MFP1[8]) and GPA_MFP[5]
 * |        |          |determine the PA.5 function.
 ...
 * |        |          |(0, 1, 1) = SC1_CLK/UART4_TXD function is selected.
 * |        |          |(1, 0, 1) = AD7 function is selected.     
 * |[7]     |GPA_MFP7  |PA.7 Pin Function Selection
 * |        |          |Bits EBI_EN (ALT_MFP[11]), PA7_SC1DAT (ALT_MFP1[6]), PA7_S21 (ALT_MFP[2]) and GPA_MFP[7]
 * |        |          |determine the PA.7 function.
 ...
 * |        |          |(0, 1, 0, 1) = SC1_DAT/UART4_RXD function is selected.
 ...
 * |        |          |Bits EBI_EN (ALT_MFP[11]), PA10_11_CAN1 (ALT_MFP[28]) and GPA_MFP[10] determine the PA.10
 * |        |          |function.
 * |        |          |(EBI_EN, PA10_11_CAN1, GPA_MFP10) value and function mapping is as following list.
 * |        |          |(0, 0, 0) = GPIO function is selected.
 * |        |          |(0, 0, 1) = I2C1_SDA function is selected.
 * |        |          |(0, 1, 1) = CAN1_TXD function is selected.
 * |        |          |(1, 0, 1) =nWR(EBI) function is selected.     
 * |[11]    |GPA_MFP11 |PA.11 Pin Function Selection
 * |        |          |Bits EBI_EN (ALT_MFP[11]), PA10_11_CAN1 (ALT_MFP[28]) and GPA_MFP[11] determine the PA.11
 * |        |          |function.
 * |        |          |(EBI_EN, PA10_11_CAN1, GPA_MFP11) value and function mapping is as following list.
 * |        |          |(0,0, 0) = GPIO function is selected.
 * |        |          |(0,0, 1) = I2C1_SCL function is selected.
 * |        |          |(0, 1, 1) = CAN1_RXD function is selected.
 * |        |          |(1, 0, 1) = nRD(EBI) function is selected.     
 * |[12]    |GPA_MFP12 |PA.12 Pin Function Selection
 * |        |          |Bits EBI_HB_EN[5] (ALT_MFP[21]), EBI_EN (ALT_MFP[11]), PA12_SC2DAT (ALT_MFP1[11]) and
 * |        |          |GPA_MFP[12] determine the PA.12 function.
 ...
 * |        |          |(0, 0, 1, 1) = SC2_DAT/UART5_RXD function is selected.
 * |        |          |(1, 1, 0, 1) = AD13 function is selected.       
 * |[13]    |GPA_MFP13 |PA.13 Pin Function Selection     
 * |        |          |Bits EBI_HB_EN[6] (ALT_MFP[22]), EBI_EN (ALT_MFP[11]), PA13_SC2CLK (ALT_MFP1[10]) and
 * |        |          |GPA_MFP[13] determine the PA.13 function.
 ...
 * |        |          |(0, 0, 1, 1) = SC2_CLK/UART5_TXD function is selected.
 * |        |          |(1, 1, 0, 1) = AD14 function is selected.        
 * |[14]    |GPA_MFP14 |PA.14 Pin Function Selection
 * |        |          |Bits EBI_HB_EN[7] (ALT_MFP[23]), EBI_EN (ALT_MFP[11]), PA14_SC2RST (ALT_MFP1[13]) and
 * |        |          |GPA_MFP[14] determine the PA.14 function.
 ...
 * |        |          |Bits PA15_SC2PWR (ALT_MFP1[12]), PA15_I2SMCLK (ALT_MFP[9]) and GPA_MFP[15] determine the PA.15
 * |        |          |function.
 * |        |          |(PA15_SC2PWR, PA15_I2SMCLK, GPA_MFP15) value and function mapping is as following list.
 * |        |          |(0, 0, 0) = GPIOA function is selected.
 ...
 * |        |          |Bits EBI_nWRL_EN (ALT_MFP[13]), EBI_EN (ALT_MFP[11]), PB2_TM2 (ALT_MFP2[4]), PB2_CPO0
 * |        |          |(ALT_MFP[30]), PB2_T2EX (ALT_MFP[26]) and GPB_MFP[2] determine the PB.2 function.
 * |        |          |(EBI_nWRL_EN, EBI_EN, PB2_TM2, PB2_CPO0, PB2_T2EX, GPB_MFP2) value and function mapping is as
 * |        |          |following list.
 * |        |          |(0, 0, 0, 0, 0, 0) = GPIO function is selected.
 * |        |          |(0, 0, 0, 0, 0, 1) = UART0_nRTS function is selected.
 * |        |          |(0, 0, 0, 0, 1, 1) = TM2_EXT function is selected.
 * |        |          |(0, 0, 0, 1, 0, 1) = ACMP0_O function is selected.
 * |        |          |(0, 0, 1, 0, 0, 1) = TM2 function is selected.
 * |        |          |(1, 1, 0, 0, 0, 1) = nWRL(EBI) function is selected.     
 * |[3]     |GPB_MFP3  |PB.3 Pin Function Selection
 * |        |          |PB3_SC2CD (ALT_MFP1[14]), PB3_T3EX (ALT_MFP[27]) and GPB_MFP[3] determine the PB.3 function.
 * |        |          |(EBI_nWRH_EN, EBI_EN, PB3_TM3, PB3_SC2CD, PB3_T3EX, GPB_MFP3) value and function mapping is as
 * |        |          |following list.
 * |        |          |(0, 0, 0, 0, 0, 0) = GPIO function is selected.
 * |        |          |(0, 0, 0, 0, 0, 1) = UART0_nCTS function is selected.
 * |        |          |(0, 0, 0, 0, 1, 1) = TM3_EXT function is selected.
 * |        |          |(0, 0, 0, 1, 0, 1) = SC2_CD function is selected.
 * |        |          |(0, 0, 1, 0, 0, 1) = TM3 function is selected.
 * |        |          |(1, 1, 0, 0, 0, 1) = nWRH(EBI) function is selected.              
 ...
 * |        |          |(0, 0) = GPIO function is selected to the pin PB.13.
 * |        |          |(0, 1) = ACMP1_O function is selected to the pin PB.13.
 * |        |          |(1, 1) = AD1 function is selected.     
 * |[14]    |GPB_MFP14 |PB.14 Pin Function Selection
 * |        |          |Bits PB14_15_EBI (ALT_MFP2[1]), PB14_S31 (ALT_MFP[3]) and GPB_MFP[14] determine the PB.14
 * |        |          |function.
 * |        |          |(PB14_15_EBI , PB14_S31, GPB_MFP14) value and function mapping is as following list
 * |        |          |(0, 0, 0) = GPIO function is selected.
 * |        |          |(0, 0, 1) = INT0 function is selected.
 * |        |          |(0, 1, 1) = SPI3_SS1 function is selected.
 * |        |          |(1, 0, 1) = AD0 function is selected.     
 * |[15]    |GPB_MFP15 |PB.15 Pin Function Selection
 * |        |          |Bits PB14_15_EBI (ALT_MFP2[1]), PB15_T0EX (ALT_MFP[24]), PB15_TM0 (ALT_MFP2[2]) and GPB_MFP[15]
 * |        |          |determine the PB.15 function.
 ...
 * |        |          |Bit GPC_MFP[9] determines the PC.5 function.
 ...
 * |        |          |Bit GPD_MFP[9] determines the PD.6 function.
 * |        |          |0 = The GPIO function is selected to the pin PD.6.
 * |        |          |1 = The CAN0_RXD function is selected to the pin PD.6.     .
 * |[7]     |GPD_MFP7  |PD.7 Pin Function Selection
 * |        |          |Bit GPD_MFP[7] determines the PD.7 function.
 * |        |          |0 = The GPIO function is selected to the pin PD.7.
 * |        |          |1 = The CAN0_TXD function is selected to the pin PD.7.     
 ...
 * |        |          |(1, 1) = CAN1_RXD function is selected.     
 ...
 * |        |          |(1, 1) = CAN1_TXD function is selected.     
 ...
 * |        |          |1 = PS/2_DAT function is selected to the pin PF.2.     
 ...
 * |        |          |1 = PS/2_CLK function is selected to the pin PF.3.     
 ...
 * |        |          |additional registers EBI_EN[7:0] and EBI_MCLK_EN for some GPIO to switch to EBI
 * |        |          |function(AD[15:8], MCLK).
 ...
 * |        |          |Bits PB2_TM2 (ALT_MFP2[4]), PB2_CPO0 (ALT_MFP[30]), PB2_T2EX (ALT_MFP[26]) and GPB_MFP[2]
 * |        |          |determine the PB.2 function.
 ...
 * |        |          |(0, 1, 0, 1) = ACMP0_O function is selected.          
 * |[27]    |PB3_T3EX  |PB.3 Pin Alternative Function Selection
 * |        |          |Bits PB3_TM3 (ALT_MFP2[5]), PB3_SC2CD (ALT_MFP1[14]), PB3_T3EX (ALT_MFP[27]) and GPB_MFP[3]
 * |        |          |determine the PB.3 function.
 ...
 * |        |          |(0, 1, 0, 1) = SC2_CD function is selected.
 * |        |          |(1, 0, 0, 1) = TM3 function is selected.            
 * |[28]    |PA10_11_CAN1|PA.10 and PA.11 Pin Alternative Function Selection
 * |        |          |Bits PA10_11_CAN1 (ALT_MFP[28]) and GPA_MFP[10] determine the PA.10 function.
 * |        |          |(PA10_11_CAN1, GPA_MFP10) value and function mapping is as following list.
 * |        |          |(0, 0) = GPIO function is selected.
 ...
 * |Bits    |Field     |Descriptions
 * | :----: | :----:   | :---- |
 * |[0]     |PD14_15_CAN1|PD.14 and PD.15 Pin Alternative Function Selection     
 * |        |          |Bits PD14_15_CAN1 (ALT_MFP2[0]) and GPD_MFP[14] determine the PD.14 function.
 * |        |          |(PD14_15_CAN1, GPD_MFP14) value and function mapping is as following list.
 * |        |          |(0, 0) = GPIO function is selected.
 * |        |          |(0, 1) = UART2_RXD function is selected.
 ...
 * |        |          |(0, 1) = UART2_TXD function is selected.
 * |        |          |(1, 1) = CAN1_TXD function is selected.    
 * |[0]     |PB14_15_EBI|PB.14 and PB.15 Pin Alternative Function Selection 
 * |        |          |Bits PB14_15_EBI (ALT_MFP2[1]), PB14_S31 (ALT_MFP[3]) and GPB_MFP[14] determine the PB.14
 * |        |          |function.
 * |        |          |(PB14_15_EBI , PB14_S31, GPB_MFP14) value and function mapping is as following list
 * |        |          |(0, 0, 0) = GPIO function is selected.
 ...
 * |        |          |The field defines that how many times of HIRC trim value is updated by auto trim circuit before
 * |        |          |the HIRC frequency locked.
 ...
 * |        |          |count reached and the internal 22.1184 MHz high speed oscillator clock frequency still doesn't
 * |        |          |be locked.
 * |        |          |Once this bit is set, the auto trim operation stopped and TRIM_SEL (IRCTRIMCTL [1:0]) will be
 * |        |          |cleared to 00 by hardware automatically.
 * |        |          |If this bit is set and TRIM_FAIL_IEN (IRCTRIMIEN[1]) is high, an interrupt will be triggered to
 * |        |          |notify that HIRC trim value update limitation count was reached.
 ...
 * |        |          |When the frequency of external 32.768 kHz low speed crystal or internal 22.1184 MHz high speed
 * |        |          |oscillator is shift larger to unreasonable value, this bit will be set and to be an indicate
 * |        |          |that clock frequency is inaccuracy
 * |        |          |Once this bit is set to 1, the auto trim operation stopped and TRIM_SEL (IRCTRIMCTL [1:0]) will
 * |        |          |be cleared to 00 by hardware automatically if CLKERR_STOP_EN (IRCTRIMCTL [8]) is set to 1.
 * |        |          |If this bit is set and CLKERR_IEN (IRCTRIMIEN [2]) is high, an interrupt will be triggered to
 * |        |          |notify the clock frequency is inaccuracy.
 * |        |          |Write 1 to clear this to 0.
 ...
 * |        |          |The NMI interrupt to Cortex-M0 can be selected from one of IRQ0~IRQ31 by setting NMI_SEL with
 * |        |          |IRQ number.
 ...
 * |        |          |0 = IRQ0~31 assigned to NMI interrupt Disabled. (NMI still can be software triggered by setting
 * |        |          |its pending flag.)
 ...
 * |        |          |The MCU_IRQ collects all the interrupts from the peripherals and generates the synchronous
 * |        |          |interrupt to Cortex-M0.
 ...
 * |        |          |0 = MCU IRQ latency is fixed at 13 clock cycles of HCLK, 
 * |        |          |    MCU will enter IRQ handler after this fixed latency when interrupt happened.
 ...
 * |        |          |By writing to this register, the UART will send out an 8-bit data through the UART_TXD pin (LSB
 * |        |          |first).
 * |        |          |By reading this register, the UART will return an 8-bit data received from UART_RXD pin (LSB
 * |        |          |first).
 * @var UART_T::THR
 ...
 * |        |          |RDA_IEN (UA_IER[0]) enabled, and an interrupt will be generated).
 ...
 * |        |          |RS485_NMM (UA_ALT_CSR[8]) is programmed.
 ...
 * |        |          |checked as logic 0.
 * |        |          |If PBE (UA_LCR[3]) is 1 and EBE (UA_LCR[4]) is 0 then the parity bit is transmitted and checked
 * |        |          |as 1.
 ...
 * |        |          |This bit is direct control internal RTS signal active or not, and then drive the RTS pin output
 * |        |          |with LEV_RTS bit configuration.
 ...
 * |        |          |when MODEM_IEN (UA_IER [3]) is set to 1.
 ...
 * |        |          |Note: This bit echoes when UART Controller peripheral clock is enabled, and CTS multi-function
 * |        |          |port is selected.
 ...
 * |        |          |If the number of bytes of received data is greater than RX_FIFO (UA_RBR) size, 64/16/16 bytes of
 * |        |          |UART0/UART1/UART2, this bit will be set.
 ...
 * |        |          |Note1: This field is used for RS-485 function mode and RS485_ADD_EN (UA_ALT_CSR[15]) is set to 1
 * |        |          |to enable Address detection mode.
 ...
 * |        |          |1 = RX FIFO is empty.
 * |        |          |Note: When the last byte of RX FIFO has been read by CPU, hardware sets this bit high. It will
 * |        |          |be cleared when UART receives any new data.
 ...
 * |        |          |When one byte of TX FIFO is transferred to Transmitter Shift Register, then TX_POINTER decreases
 * |        |          |one.
 ...
 * |        |          |As one byte of TX FIFO is transferred to Transmitter Shift Register, the TX_FULL bit is cleared
 * |        |          |to 0 and TX_POINTER will show 63/15/15 (UART0/UART1/UART2).
 * |[22]    |TX_EMPTY  |Transmitter FIFO Empty (Read Only)
 * |        |          |This bit indicates TX FIFO empty or not.
 * |        |          |0 = TX FIFO is not empty.
 * |        |          |1 = TX FIFO is empty.
 * |        |          |Note: When the last byte of TX FIFO has been transferred to Transmitter Shift Register, hardware
 ...
 * |        |          |This bit is set by hardware when TX FIFO (UA_THR) is empty and the STOP bit of the last byte has
 * |        |          |been transmitted.
 * |        |          |0 = TX FIFO is not empty.
 * |        |          |1 = TX FIFO is empty.
 * |        |          |Note: This bit is cleared automatically when TX FIFO is not empty or the last byte transmission
 * |        |          |has not completed.
 * @var UART_T::ISR
 ...
 * |        |          |drops below the threshold level RFITL(UA_FCR[7:4]).
 ...
 * |        |          |Note1: In RS-485 function mode, this field is set include receiver detect and received address
 * |        |          |byte character (bit9 = '1') bit. At the same time, the bit of RS485_ADD_DETF(UA_FSR[3]) is also
 * |        |          |set.
 * |        |          |Note2: This bit is read only and reset to 0 when all bits of BIF(UA_FSR[6]), FEF(UA_FSR[5]) and
 * |        |          |PEF(UA_FSR[4]) are cleared.
 ...
 * |        |          |This bit is set when the RX FIFO is not empty and no activities occurred in the RX FIFO and the
 * |        |          |time-out counter equal to TOIC(UA_TOR[7:0]).
 ...
 * |        |          |This bit is set when LIN slave header detect (LINS_HDET_F (UA_LIN_SR[0] = 1)), LIN break detect
 * |        |          |(LIN_BKDET_F(UA_LIN_SR[9]=1)), 
 * |        |          |bit error detect (BIT_ERR_F(UA_LIN_SR[9])=1), LIN slave ID parity error
 * |        |          |(LINS_IDPERR_F(UA_LIN_SR[2]) = 1) or LIN slave header error detect (LINS_HERR_F (UA_LIN_SR[1])).
 ...
 * |        |          |LIN_BKDET_F(UA_LIN_SR[9]), BIT_ERR_F(UA_LIN_SR[9]), LINS_IDPENR_F (UA_LIN_SR[2]) and
 * |        |          |LINS_HERR_F(UA_LIN_SR[1]) all are cleared.
 ...
 * |        |          |Note: This bit is read only and reset to 0 when the bit DCTSF(US_MSR[0]) is cleared by writing 1
 * |        |          |on DCTSF (US_MSR[0]).
 ...
 * |        |          |(UA_LIN_SR [0]) flag will be asserted. If the LIN_IEN (UA_IER[8]) = 1, an interrupt will be
 * |        |          |generated.
 ...
 * |        |          |0 = UA_BAUD updated is written by software (if no automatic resynchronization update occurs at
 * |        |          |the same time).
 ...
 * |        |          |and LIN_HEAD_SEL (UA_LIN_CTL[23:22]) = 10 or be used for enable LIN slave received frame ID
 * |        |          |parity checked.
 ...
 * |        |          |character, the LIN_BKDET_F (UA_LIN_SR[8]) flag is set in UA_LIN_SR register at the end of break
 * |        |          |field.
 ...
 * |        |          |calculate P0 (LIN_PID[30]) and P1 (LIN_PID[31]), otherwise user must filled frame ID and parity
 * |        |          |in this field.
 ...
 * |        |          |This bit is set by hardware when a LIN header is detected in LIN slave mode and be cleared by
 * |        |          |writing 1 to it.
 ...
 * |        |          |header ("break + sync + frame ID"), the LINS_HEDT_F will be set whether the frame ID correct or
 * |        |          |not.
 ...
 * |        |          |"sync field measure time-out with Automatic Resynchronization mode" and "LIN header reception
 * |        |          |time-out".
 ...
 * |        |          |This bit is set by hardware when a break is detected and be cleared by writing 1 to it through
 * |        |          |software.
 ...
 * |        |          |Note1: This bit iscan be cleared by writing 1 to it.
 * |        |          |Note2: This bit is only valid when enable bit error detection function (BIT_ERR_EN (UA_LIN_CTL
 * |        |          |[12]) = 1).
 */
